...
var (
	articleDBTypes = map[string]string{`Body`: `text`, `Categories`: `ARRAYtext`, `CreatedAt`: `timestamp with time zone`, `ID`: `bigint`, `Published`: `boolean`, `Slug`: `text`, `Summary`: `text`, `Title`: `text`, `UpdatedAt`: `timestamp with time zone`, `UpdatedBy`: `text`}
	_              = bytes.MinRead
)
