...
var (
	eventDBTypes = map[string]string{`Body`: `text`, `Categories`: `ARRAYtext`, `ContactEmail`: `text`, `ContactPerson`: `text`, `ContactPhone`: `text`, `ContactURL`: `text`, `CreatedAt`: `timestamp with time zone`, `EventDate`: `timestamp with time zone`, `EventLocation`: `text`, `EventTime`: `text`, `ID`: `bigint`, `Published`: `boolean`, `Slug`: `text`, `Summary`: `text`, `Title`: `text`, `UpdatedAt`: `timestamp with time zone`, `UpdatedBy`: `text`}
	_            = bytes.MinRead
)
...
func testEventsUpsert(t *testing.T) {
	...
	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = event.Upsert(tx, false, nil, nil); err != nil {
	...
	if err = event.Upsert(tx, true, nil, nil); err != nil {
		t.Errorf("Unable to upsert Event: %s", err)
	}
