...
var (
	userDBTypes = map[string]string{`ConfirmedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`, `EmailAddress`: `text`, `Enabled`: `boolean`, `EncryptedPassword`: `text`, `EncryptedSalt`: `text`, `FirstName`: `text`, `ID`: `bigint`, `LastName`: `text`, `PasswordResetAt`: `timestamp with time zone`, `Prefs`: `jsonb`, `Role`: `integer`, `Summary`: `text`, `UpdatedAt`: `timestamp with time zone`, `UpdatedBy`: `text`, `Username`: `text`}
	_           = bytes.MinRead
)
