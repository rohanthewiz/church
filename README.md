...
A template-less golang Content Management System (CMS) based on the RWeb web framework (https://github.com/rohanthewiz/rweb) and SQLBoiler (https://github.com/vattle/sqlboiler) and an innovative, think-outside-the-box golang architecture. 
...
## Architecture
- The **RWeb** web framework (https://github.com/rohanthewiz/rweb) - light, low-dependency, and fast (the platform originally ran on Echo; that stack has been fully removed)
- **Postgres** is our main database. **SQLBoiler** provides a code-generated, thus compiled and performant database interface layer
- Sessions and tokens are held in an **in-process key-value store** (`core/kvstore`) - no external session store (Redis) is required
